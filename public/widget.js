/**
 * AI Asuka Stylist — Embeddable Chat Widget
 * 
 * Add to Shopify theme with:
 * <script src="https://YOUR-VERCEL-URL.vercel.app/widget.js" defer></script>
 * 
 * This is a self-contained widget that creates a shadow DOM 
 * to avoid CSS conflicts with the host site.
 */
(function () {
  "use strict";
      ...
      <button class="asuka-bubble" aria-label="Open AI Asuka Stylist">
        ${
          state.open
            ...
            : `<span class="bubble-logo">A</span><span class="bubble-ai">AI</span>
               <span class="bubble-dot"></span>`
        }
      </button>
      ${
        !state.open
          ? `<div class="asuka-tooltip">Need styling help? <em>Ask Asuka AI</em></div>`
          : ""
      }
    `;
  }
    ...
    return `
      <div class="asuka-panel">
        ...
        <div class="asuka-input-area">
          <div class="input-row">
            <textarea class="asuka-textarea" placeholder="${
              state.mode === "style"
                ? "Tell me about your occasion..."
                : "Describe your dream design..."
            }" rows="1">${state.input}</textarea>
  ...
  function renderWelcome() {
      ...
      return `
        <div class="welcome">
          ...
          <div class="prompt-pills">
            ${STYLE_PROMPTS.map(
              (p) => `<button class="prompt-pill" data-prompt="${p}">${p}</button>`
            ).join("")}
          </div>
        </div>
      `;
    ...
    return `
      <div class="welcome">
        ...
        <p class="welcome-sub">Describe your dream outfit — I'll create a design concept for you.</p>
        <div class="prompt-list">
          ${DESIGN_PROMPTS.map(
            (p) =>
              `<button class="prompt-card" data-prompt="${p}"><span class="prompt-star">✦</span> ${p}</button>`
          ).join("")}
        </div>
      </div>
    `;
  ...
  function renderMessage(msg) {
    ...
    if (msg.products?.length) {
      ...
      <div class="bridge-text">
        Want something unique? <button class="bridge-link" data-switch="design">Create a custom design ✦</button>
      </div>`;
    }
        ...
        <div class="design-card">
          ...
          <div class="design-info">
            ...
            <div class="design-actions">
              ...
              <button class="design-btn secondary" data-prompt="I'd like to refine this design">Refine</button>
            </div>
          </div>
        </div>`;
    }
    ...
    if (msg.stores) {
      ...
      const storeEntries = stores.name
        ? [stores]
    ...
    return `
      <div class="msg-row msg-bot">
        ...
        <div class="msg-content">
          <div class="msg-bubble bot-bubble">${escapeHtml(msg.text)}</div>
          ${extra}
        </div>
      </div>
    `;
  ...
  function attachEvents() {
    // Bubble toggle
    const bubble = shadow.querySelector(".asuka-bubble");
    ...
    shadow.querySelectorAll("[data-prompt]").forEach((btn) => {
      btn.addEventListener("click", () => {
        if (btn.classList.contains("design-btn")) {
          // Refine button
    ...
    if (textarea) {
      ...
      textarea.addEventListener("input", (e) => {
        ...
        if (sendBtn)
          sendBtn.classList.toggle("active", state.input.trim().length > 0);
      });
      textarea.addEventListener("keydown", (e) => {
        if (e.key === "Enter" && !e.shiftKey) {
          e.preventDefault();
          if (state.input.trim()) {
            sendMessage(state.input.trim());
            state.input = "";
          }
        }
      });
    ...
    return `
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .asuka-bubble {
        ...
        box-shadow: 0 4px 20px rgba(139,101,77,0.35);
      ...
      .bubble-logo {
        ...
        color: ${CR}; font-weight: 600; line-height: 1;
      }
      ...
      .asuka-tooltip::after {
        ...
        border: 1px solid ${CK}; border-top: none; border-left: none;
      ...
      .asuka-panel {
        ...
        height: 580px; max-height: 76vh;
        background: ${W}; border-radius: 18px;
        border: 1px solid ${CK};
      ...
      .header-icon {
        ...
        background: ${B}; display: flex; align-items: center; justify-content: center;
        font-family: 'Playfair Display', serif; font-size: 16px;
        color: ${CR}; font-weight: 600;
      }
      .header-title {
        font-size: 14.5px; color: ${TD}; font-weight: 600;
        display: flex; align-items: center; gap: 7px;
      }
      .ai-badge {
        font-size: 8.5px; background: ${B}; color: ${CR};
        padding: 2px 6px; border-radius: 4px; font-weight: 700;
        letter-spacing: 1px;
      }
      .header-sub {
        font-size: 10px; color: ${TL}; margin-top: 1px;
        display: flex; align-items: center; gap: 4px;
      }
      .online-dot { width: 5px; height: 5px; border-radius: 50%; background: #5a9a5a; }
      .close-btn {
        background: none; border: none; cursor: pointer;
        color: ${TL}; font-size: 22px; line-height: 1;
      }

      /* Mode Tabs */
      .mode-tabs {
        display: flex; background: ${CD}; border-radius: 9px; padding: 3px;
      }
      .mode-tab {
        flex: 1; background: transparent; border: 1px solid transparent;
        border-radius: 7px; padding: 8px 6px; cursor: pointer; text-align: center;
        transition: all 0.25s;
      }
      .mode-tab.active {
        background: ${W}; border-color: ${CK};
        box-shadow: 0 1px 3px rgba(139,101,77,0.06);
      }
      ...
      .welcome-title {
        ...
        color: ${TD}; margin-bottom: 6px;
      }
      .welcome-title em { color: ${B}; }
      .welcome-sub {
        font-size: 12px; color: ${TM}; line-height: 1.6;
        font-weight: 300; max-width: 260px; margin: 0 auto 16px;
      }
      ...
      .bot-avatar {
        ...
        color: ${CR}; font-weight: 600; flex-shrink: 0;
      ...
      .product-cta {
        ...
        font-size: 10px; color: ${B}; font-weight: 600;
      }

      .bridge-text {
        margin-top: 6px; font-size: 11px; color: ${TM};
      }
      .bridge-link {
        background: none; border: none; color: ${B};
      ...
      /* Design Card */
      .design-card {
        margin-top: 8px; background: linear-gradient(135deg, ${CD}, rgba(139,101,77,0.06));
        border: 1px solid ${CK}; border-radius: 12px; overflow: hidden;
      }
      .design-preview {
        height: 120px; display: flex; flex-direction: column;
        align-items: center; justify-content: center;
        background: linear-gradient(135deg, rgba(139,101,77,0.08), rgba(139,101,77,0.02));
      }
      .design-icon {
        width: 48px; height: 48px; border-radius: 50%;
        background: rgba(139,101,77,0.1); border: 1.5px solid rgba(139,101,77,0.2);
        display: flex; align-items: center; justify-content: center;
        font-size: 18px; color: ${B};
      }
      .design-label {
        font-size: 9px; color: ${B}; margin-top: 6px;
      ...
      .design-btn {
        flex: 1; border-radius: 7px; padding: 8px;
      ...
      .asuka-input-area {
        ...
        border-top: 1px solid ${CK};
      }
      .input-row {
        display: flex; align-items: flex-end; gap: 8px;
        background: ${CR}; border: 1px solid ${CK};
      ...
      .asuka-textarea {
        ...
        min-height: 28px; max-height: 72px;
      }
      ...
      .send-btn.active {
        background: ${B}; color: ${CR}; cursor: pointer;
      }
      .powered-by {
        text-align: center; margin-top: 6px;
        font-size: 9px; color: ${TL}; letter-spacing: 0.3px;
      }
