    ...
    if (clientProducts?.length) {
      productContext = "\n\n[LIVE PRODUCTS FROM ASUKACOUTURE.COM - Show these to the customer with styling tips]\n" +
        clientProducts.map(p =>
          `• ${p.title} — ${p.price}${p.onSale ? ` (was ${p.compareAtPrice}, ${p.discount}% OFF!)` : ""} — ${p.url}`
        ).join("\n") +
